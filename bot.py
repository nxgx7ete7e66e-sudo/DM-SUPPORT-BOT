...
class QuickReplyButton(discord.ui.Button):
    def __init__(self, user_id: int):
        super().__init__(
            ...
            custom_id=f"quick_reply_{user_id}"
        )
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if staff_role not in interaction.user.roles:
            await interaction.response.send_message(
                "❌ You don't have permission to reply to tickets.", ephemeral=True
            )
            return
        # Extract user_id from custom_id in case of restart
        try:
            uid = int(self.custom_id.replace("quick_reply_", ""))
        except Exception:
            uid = self.user_id
...
class CloseTicketButton(discord.ui.Button):
    def __init__(self, user_id: int):
        super().__init__(
            ...
            custom_id=f"close_ticket_{user_id}"
        )
        self.user_id = user_id
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        # Extract user_id from custom_id in case self.user_id is 0 after restart
        try:
            uid = int(self.custom_id.replace("close_ticket_", ""))
        except Exception:
            uid = self.user_id

        if uid == 0 or uid not in active_tickets:
            await interaction.response.send_message("❌ Ticket is already closed or could not be found.", ephemeral=True)
            return
