...
@tree.command(name="clearlogs", description="Clear all messages in ticket logs channel")
@app_commands.checks.has_permissions(administrator=True)
async def clearlogs(interaction: discord.Interaction):
    await interaction.response.defer(ephemeral=True)
    log_channel = bot.get_channel(TICKET_LOG_CHANNEL)
    if not log_channel:
        await interaction.followup.send("❌ Could not find ticket logs channel.", ephemeral=True)
        return
    # Use bulk delete for messages under 14 days (much faster, no timeout)
    try:
        messages = [m async for m in log_channel.history(limit=200)]
        if messages:
            await log_channel.delete_messages(messages)
        await interaction.followup.send(f"✅ Cleared {len(messages)} messages from ticket logs.", ephemeral=True)
...
async def commands_list(interaction: discord.Interaction):
    nl = chr(10)
    user_cmds = "!close - Close your ticket (use in DM)"
    staff_cmds = "/reply - Reply by user ID" + nl + "Quick Reply button - Reply from log" + nl + "Close Ticket button - Close from log"
    admin_cmds = "/panel - Post support panel" + nl + "/clearlogs - Clear ticket logs" + nl + "/commands - Show this help"
    embed = discord.Embed(title="UH Support Bot Commands", color=discord.Color.blurple())
