...
async def clearlogs(interaction: discord.Interaction):
    ...
    try:
        ...
        if not messages:
            await interaction.followup.send("Ticket logs already empty.", ephemeral=True)
            return
        try:
            ...
            for m in messages:
                try:
                    ...
                    pass
        count = len(messages)
        await interaction.followup.send(f"Cleared {count} messages from ticket logs.", ephemeral=True)
