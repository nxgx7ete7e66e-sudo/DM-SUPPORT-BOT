...
# Active tickets loaded from disk so they survive restarts
...
class IssueModal(discord.ui.Modal, title="Describe Your Issue"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if log_channel:
            ...
            log_embed.add_field(name="📋 User ID (tap & hold to copy)", value=f"`{user.id}`", inline=False)
            log_embed.add_field(name="Issue", value=self.issue.value, inline=False)
            log_embed.add_field(
                name="💬 Reply Command",
                value=f"`/reply user_id: {user.id} message: your reply here`",
                inline=False
            )
