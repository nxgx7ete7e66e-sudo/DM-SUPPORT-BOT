...
async def commands_list(interaction: discord.Interaction):
    embed = discord.Embed(
        title="🤖 UH Support Bot — Commands",
        color=discord.Color.blurple(),
    )
    embed.add_field(
        name="👤 User Commands",
        value=(
            "`!close` — Close your support ticket (use in DM)
"
        ),
        inline=False
    )
    embed.add_field(
        name="🎫 Staff Commands",
        value=(
            "`/reply` — Reply to a user's ticket by ID
"
            "💬 **Quick Reply** button — Reply directly from ticket log
"
            "🔒 **Close Ticket** button — Close ticket from ticket log
"
        ),
        inline=False
    )
    embed.add_field(
        ...
        value=(
            "`/panel` — Post the support panel in a channel
"
            "`/clearlogs` — Clear all messages in ticket logs
"
